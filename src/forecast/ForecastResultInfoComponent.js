        ...
        <Stack direction="row" spacing={2} sx={{ alignItems: 'center', justifyContent: 'center' }}>
          {isCollection
            ? <Box className='IconImageBox'>
                <img className='IconImage' src={`${process.env.PUBLIC_URL}/assets/nms/${result.spawn.image}`} alt={ result.spawn.name } />
              </Box> : null
          }
          <Typography variant='h6'>{text}</Typography>
        </Stack>
        <Box>
          {Array.isArray(result.condition) ? <ForecastResultInfoWeatherComponent conditions={result.condition} /> : null }
        </Box>
        <Box>
          {conflictText !== null
