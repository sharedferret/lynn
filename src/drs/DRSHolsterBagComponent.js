...
export default function DRSHolsterBagComponent({ holsterData, bagType, handleHolsterUpdate }) {
  function handleLostActionUpdate(data, index) {
  ...
  return (
    <Box sx={{ border: 1, borderRadius: '12px', borderColor: '#ddd' }}>
      <Stack>
        <Typography fontSize={30} fontWeight={700}>{bagType === 'prepop' ? 'Pre-Pop' : 'Main'}</Typography>
        <Stack spacing={1} p={1}>
          {actions.map((i, index) => <DRSBagLostActionComponent actionName={ i.name } actionQuantity={ i.quantity } index={ index } handleLostActionUpdate={ handleLostActionUpdate} key={'action-' + uuidv4()} />)}
